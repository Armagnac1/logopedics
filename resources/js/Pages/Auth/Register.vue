...
<template>
    <Head :title="$t('passwordForm.registration')" />
    ...
    <AuthenticationCard>
        ...
        <form @submit.prevent="submit">
            <div>
                <InputLabel for="name" :value="$t('passwordForm.name')" />
                <TextInput
                    id="name"
                    v-model="form.name"
                    type="text"
            ...
            <div class="mt-4">
                <InputLabel for="email" :value="$t('passwordForm.email')" />
                <TextInput
                    id="email"
                    v-model="form.email"
                    type="email"
            ...
            <div class="mt-4">
                <InputLabel for="password" :value="$t('commopasswordFormn.password')" />
                <TextInput
                    id="password"
            ...
            <div class="flex items-center justify-end mt-4">
                <Link :href="route('login')" class="underline text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800">
                    {{ $t('passwordForm.alreadyRegistered') }}
                </Link>

                <PrimaryButton class="ms-4" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">
                    {{ $t('passwordForm.register') }}
                </PrimaryButton>
            </div>
        </form>
    </AuthenticationCard>
