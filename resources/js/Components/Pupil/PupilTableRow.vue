...
<template>
    <tr>
        ...
        <td class="size-px whitespace-nowrap">
            <div class="ps-6 lg:ps-3 xl:ps-0 pe-6 py-3">
                <div class="flex items-center gap-x-3">
                    <img class="inline-block size-[38px] rounded-full" :src="pupil.profile_photo_path" alt="Аватар">
                    <div class="grow">
                        <span class="block text-sm font-semibold text-gray-800 dark:text-gray-200">
                            <Link :href="route('pupil.show', pupil.id)">
                                {{ pupil.full_name }}
                            </Link>
                        </span>
                    </div>
        ...
        <td class="size-px whitespace-nowrap">
            <div class="px-6 py-3">
                <div class="flex items-center gap-x-3">
                    <span class="text-xs text-gray-500">{{pupil.lesson_finished_count}}/{{pupil.lesson_count}}</span>
                    <div class="flex w-full h-1.5 bg-gray-200 rounded-full overflow-hidden dark:bg-gray-700">
                        <div class="flex flex-col justify-center overflow-hidden bg-gray-800 dark:bg-gray-200"
                             role="progressbar" :style="'width: ' + pupil.lesson_finished_count / pupil.lesson_count * 100 +'%'" :aria-valuenow="pupil.lesson_finished_count / pupil.lesson_count * 100" aria-valuemin="0"
        ...
        <td class="size-px whitespace-nowrap">
            <div class="px-6 py-3">
                <span v-if="pupil.next_lesson"
                      class="text-sm text-gray-500">{{ dayjs(pupil.next_lesson.start_at).format('LLL') }}</span>
            </div>
        </td>
        <td class="size-px whitespace-nowrap">
            <div class="px-6 py-1.5">
                <a v-if="pupil.next_lesson"
                   class="inline-flex items-center gap-x-1 text-sm text-blue-600 decoration-2 hover:underline font-medium dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600"
                   href="#">

                    <Link as="button" :href="route('lesson.show', pupil.next_lesson.id)">
                        К занятию
                    </Link>
                </a>
            </div>
